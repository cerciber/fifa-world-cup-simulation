...
CREATE TABLE coloresequipo (
    id_colores_equipo   INT IDENTITY(1,1)
...
CREATE TABLE terrenosentrenamiento (
    id_terrenosentrenamiento   INT IDENTITY(1,1)
...
CREATE TABLE sesiones_entrenamiento (
    id_entrenamiento   INT IDENTITY(1,1)
...
CREATE TABLE sesiones_calentamiento (
    id_calentamiento  INT IDENTITY(1,1)
...
CREATE TABLE controlantidopaje (
    id_controlantidopaje   INT IDENTITY(1,1)
...
CREATE TABLE log_acciones (
    id_log_acciones   INT IDENTITY(1,1)
...
as
	select ROW_NUMBER() OVER(ORDER BY Pts DESC, DG DESC, G DESC) as Posicion,id_equipo, Pts, J, G, E, P, GF, GC, DG  from 

(select id_equipo, ((G*3)+E) as Pts, J, G, E, P, GF, GC, (GF-GC) as DG from 
        (select id_equipo,
                (select count(id_partido) from  v_equipo_partido inner join v_partidos on id_partido = partido where equipo = id_equipo) as J,
                ...
                (select sum(goles) from v_equipo_partido where equipo = id_equipo) as GF,
...
create procedure ingresarEstadioPartido
	...
	@estadio int
as
    IF (select count(*) from v_partidos where id_partido = @id_partido) = 0 execute ingresarPartido @id_partido;
...
as
    select id_partido, id_estadio, v_estadios.nombre as estadio, fecha_hora, techo_retractil as uso_techo_retractil, tiempo_compensacion,
        ...
        equipopartido1.tiros_libres_indirectos as tiros_libres_directos1, equipopartido2.tiros_libres_indirectos as tiros_libres_directos2,
...
as
    ...
    set @asociacionOrganizadora = (select id_equipo from v_equipos where nombre = 'Rusia');
    insert into v_gastos values('Equipo', @tipo, @cantidad, @descripcion);
    insert into gasto_equipo values(SCOPE_IDENTITY(), @asociacionOrganizadora);
go
...
as
	insert into v_gastos values('Equipo', @tipo, @cantidad, @descripcion);
	insert into gasto_equipo values(SCOPE_IDENTITY(), @equipo);
go
...
go
CREATE TRIGGER tr_asignarPartidos ON equipo_partido AFTER UPDATE AS 

	declare @partido int;
	set @partido = (select partido from inserted);

	declare @equipo int;
	set @equipo = (select equipo from inserted);

	if @partido >= 49 and @partido <= 62
	begin

		declare @equipo1 int;
		set @equipo1 = (select top 1 equipo from v_equipo_partido where partido = @partido and posicion = 1);

		declare @equipo2 int;
		set @equipo2 = (select top 1 equipo from v_equipo_partido where partido = @partido and posicion = 2);

		declare @goles1 int;
		set @goles1 = (select goles from v_equipo_partido where partido = @partido and equipo = @equipo1);

		declare @goles2 int;
		set @goles2 = (select goles from v_equipo_partido where partido = @partido and equipo = @equipo2);

		if @goles1 is not null and  @goles2 is not null
		begin

			declare @siguientePartido int;
			set @siguientePartido = CASE @partido
			...
			set @posSiguientePartido = CASE @partido
...
as
	...
	BEGIN
		...
		BEGIN
			DECLARE @cedula INT;
			SET @cedula = ISNULL((select max(cedula) from V_jugadores), 0 ) + 1;
			DECLARE @apellido VARCHAR(60);
			SET @apellido = ceatorresar.p_generar_apellido(ROUND(((19 - 1) * RAND()), 0));
			DECLARE @nombre VARCHAR(60);
			SET @nombre = ceatorresar.p_generar_nombre(ROUND(((19 - 1) * RAND()), 0));
			DECLARE @pasaporte VARCHAR(60);
			SET @pasaporte = 'Pasaporte' + CONVERT(varchar(4),@cedula);
			DECLARE @equipo VARCHAR(60);
			SET @equipo = ceatorresar.p_generar_pais(@cnt1 - 1);
			DECLARE @estado_civil VARCHAR(60);
			SET @estado_civil = ceatorresar.p_generar_estado_civil(ROUND(((5 - 1) * RAND()), 0));
...
as
	...
	BEGIN
		...
		BEGIN
			DECLARE @cedula INT;
			SET @cedula = ISNULL((select max(cedula) from v_arbitros), 0 ) + 1;
			DECLARE @apellido VARCHAR(60);
			SET @apellido = ceatorresar.p_generar_apellido(ROUND(((19 - 1) * RAND()), 0));
			DECLARE @nombre VARCHAR(60);
			SET @nombre = ceatorresar.p_generar_nombre(ROUND(((19 - 1) * RAND()), 0))
			...
			SET @pais = ceatorresar.p_generar_pais(@cnt1 - 1);
			DECLARE @estado_civil VARCHAR(60);
			SET @estado_civil = ceatorresar.p_generar_estado_civil(ROUND(((5 - 1) * RAND()), 0));
...
as
	...
	BEGIN
		...
		set @federacion = ceatorresar.p_generar_confederacion(ROUND(((5 - 1) * RAND()), 0));

		DECLARE @nombre varchar(60);
		set @nombre = ceatorresar.p_generar_pais(@cnt1 - 1);

		DECLARE @acronimo varchar(60);
		set @acronimo = UPPER (SUBSTRING ( @nombre ,0 , 4));

		DECLARE @presidente varchar(60);
		set @presidente = ceatorresar.p_generar_nombre(ROUND(((19 - 1) * RAND()), 0)) + ' ' + ceatorresar.p_generar_apellido(ROUND(((19 - 1) * RAND()), 0));

		DECLARE @secretario_general varchar(60);
		set @secretario_general = ceatorresar.p_generar_nombre(ROUND(((19 - 1) * RAND()), 0)) + ' ' + ceatorresar.p_generar_apellido(ROUND(((19 - 1) * RAND()), 0));
...
as
	...
	BEGIN
		...
		BEGIN
			DECLARE @cedula INT;
			SET @cedula = (
				select top 1 cedula from V_jugadores left join v_jugador_equipo on cedula = jugador 
...
as
	...
	BEGIN
		...
		BEGIN
			DECLARE @equipo INT;
			SET @equipo = (
				select top 1 id_equipo from V_equipos left join grupo_equipo on id_equipo = equipo 
...
as
	...
	BEGIN
		...
		BEGIN
			...
			SET @colores = ceatorresar.p_generar_color(ROUND(((10 - 1) * RAND()), 0));
...
as
	...
	BEGIN
		declare @estadio int;
		set @estadio = isnull(ROUND(((20 - 1) * RAND()), 0) + 1, 1);
...
as
	...
	BEGIN
		...
		DECLARE @cedula1 INT;
		SET @cedula1 = (select top 1 cedula from V_arbitros order by newid());
		execute ingresarArbitroPartido @cedula1, @cnt1, 'Principal';

		DECLARE @cedula2 INT;
		SET @cedula2 = (select top 1 cedula from V_arbitros where cedula <> @cedula1 order by newid());
		execute ingresarArbitroPartido @cedula2, @cnt1, 'Arbitro asistente 1';

		DECLARE @cedula3 INT;
		SET @cedula3 = (select top 1 cedula from V_arbitros where cedula <> @cedula1 and cedula <> @cedula2 order by newid());
		execute ingresarArbitroPartido @cedula3, @cnt1, 'Arbitro asistente 2';

		DECLARE @cedula4 INT;
		SET @cedula4 = (select top 1 cedula from V_arbitros where cedula <> @cedula1 and cedula <> @cedula2 and cedula <> @cedula3 order by newid());
		execute ingresarArbitroPartido @cedula4, @cnt1, 'Cuarto arbitro';

		DECLARE @cedula5 INT;
		SET @cedula5 = (select top 1 cedula from V_arbitros where cedula <> @cedula1 and cedula <> @cedula2 and cedula <> @cedula3 and cedula <> @cedula4 order by newid());
...
as
	...
	BEGIN
		...
		declare @equipo1 int;
		set @equipo1 = (select top 1 equipo from v_equipo_partido where partido = @cnt1 and posicion = 1);

		declare @equipo2 int;
		set @equipo2 = (select top 1 equipo from v_equipo_partido where partido = @cnt1 and posicion = 2);

		declare @color_Jugador1 int;
		set @color_Jugador1 = (select top 1 id_colores_equipo from V_coloresequipo where equipo = @equipo1 and portador = 'Jugador');
		declare @color_guardameta1 int;
		set @color_guardameta1 = (select top 1 id_colores_equipo from V_coloresequipo where equipo = @equipo1 and portador = 'Guardameta');
		execute ingresarColoresDeJuego @equipo1, @cnt1, @color_Jugador1, @color_guardameta1;

		declare @color_Jugador2 int;
		set @color_Jugador2 = (select top 1 id_colores_equipo from V_coloresequipo where equipo = @equipo2 and portador = 'Jugador');
		declare @color_guardameta2 int;
		set @color_guardameta2 = (select top 1 id_colores_equipo from V_coloresequipo where equipo = @equipo2 and portador = 'Guardameta');
...
as
	...
	BEGIN
		...
		declare @equipo1 int;
		set @equipo1 = (
			select top 1 equipo from v_equipo_partido 
		...
		BEGIN
			...
			set @jugador1 = (
				select top 1 t1.jugador from (select jugador from v_jugador_equipo where equipo = @equipo1) t1
				left join (
			...
			set @jugador2 = (
				select top 1 t1.jugador from (select jugador from v_jugador_equipo where equipo = @equipo2) t1
				left join (
					select v_titulares_partido.jugador from v_titulares_partido 
					inner join jugador_equipo on v_titulares_partido.jugador = jugador_equipo.jugador
...
as
	...
	BEGIN
		...
		set @techo_retractil = ROUND(RAND(), 0);

		declare @tiempo_compensacion int;
		set @tiempo_compensacion = ROUND(((5) * RAND()), 0);

		declare @tiempo_extra int;
		set @tiempo_extra = ROUND(RAND(), 0);

		declare @rondas_penales int;
		set @rondas_penales = ROUND(RAND(), 0);
		...
		declare @equipo int;
		set @equipo = (select top 1 equipo from v_equipo_partido where partido = @cnt1 and posicion = 1);

		declare @goles int;
		set @goles = ROUND(((5) * RAND()), 0);

		declare @tiros_libres_directos int;
		set @tiros_libres_directos = ROUND(((15) * RAND()), 0);

		declare @tiros_libres_indirectos int;
		set @tiros_libres_indirectos = ROUND(((15) * RAND()), 0);

		declare @penales int;
		set @penales = ROUND(((3) * RAND()), 0);
		if @rondas_penales = 1 set @penales = @penales + 5;

		execute ingresarResultadoEquipoPartido @equipo, @cnt1, @goles, @tiros_libres_directos, @tiros_libres_indirectos, @penales

		set @equipo = (select top 1 equipo from v_equipo_partido where partido = @cnt1 and posicion = 2);
		set @goles = ROUND(((5) * RAND()), 0);
		set @tiros_libres_directos = ROUND(((15) * RAND()), 0);
		set @tiros_libres_indirectos = ROUND(((15) * RAND()), 0);
		set @penales = ROUND(((15) * RAND()), 0);
...
as
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 1 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);

	set @equipo2 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 1 order by v_tablaDePosiciones.Posicion asc) t1
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 2 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 3 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 4 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 5 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 6 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 7 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);
	...
	set @equipo1 = (select top 1 id_equipo from (select top 2 id_equipo, v_tablaDePosiciones.posicion from v_tablaDePosiciones
		inner join v_grupo_equipo on id_equipo = equipo
		where grupo = 8 order by v_tablaDePosiciones.Posicion asc) t1
		order by t1.Posicion asc);
...
as
	...
	WHILE @cnt1 <= @NumGastos
		BEGIN
			...
			SET @cantidad = ROUND(((3000 - 1) * RAND()),0);

			DECLARE @num INT;
			SET @num = ROUND(((9 - 1) * RAND()),0);

			DECLARE @tipo VARChAR(60);
			SET @tipo = ceatorresar.p_generar_tipo_gasto_FIFA (@num);

			DECLARE @descripcion VARChAR(255);
			SET @descripcion = ceatorresar.p_generar_descripcion_gasto_FIFA (@num);
...
as
	...
	WHILE @cnt1 <= @NumGastos
		BEGIN
			...
			DECLARE @num INT;
			SET @num = ROUND(((5 - 1) * RAND()),0);

			DECLARE @tipo VARChAR(60);
			SET @tipo = ceatorresar.p_generar_tipo_gasto_Equipos (@num);

			DECLARE @descripcion VARChAR(255);
			SET @descripcion = ceatorresar.p_generar_descripcion_gasto_Equipos (@num);

			DECLARE @equipo INT;
			SET @equipo = ROUND(((32 - 1) * RAND()),0) + 1;
...
as
	...
	WHILE @cnt1 <= @NumIncidentes
		BEGIN
			...
			DECLARE @partido INT;
			SET @partido = (select top 1 id_partido from V_partidos order by newid());

			DECLARE @arbitro INT;
			SET @arbitro = (select top 1 arbitro from v_arbitro_partido where partido = @partido order by newid());

			DECLARE @num INT;
			SET @num = ROUND(((5 - 1) * RAND()),0);

			DECLARE @incidente VARChAR(60);
			SET @incidente = ceatorresar.p_generar_inicidente (@num);

			DECLARE @descripcion VARChAR(255);
			SET @descripcion = ceatorresar.p_generar_descripcion_inicidente (@num);

			SET @num = ROUND((2 * RAND()),0);
...
as
	...
	WHILE @cnt1 <= @NumFaltas
		BEGIN
			...
			DECLARE @partido INT;
			SET @partido = (select top 1 partido from v_titulares_partido order by newid());

			DECLARE @jugador INT;
			SET @jugador = (select top 1 jugador from v_titulares_partido where partido = @partido order by newid());

			DECLARE @num INT;
			SET @num = ROUND(((5 - 1) * RAND()),0);

			DECLARE @mensaje VARChAR(60);
			SET @mensaje = ceatorresar.p_generar_mensaje_falta(@num);

			DECLARE @sancion datetime;
			SET @sancion = (select top 1 fecha_hora from V_partidos where id_partido = @partido order by newid());
			SET @sancion = DATEADD(day, ROUND(((60 - 1) * RAND()) + 10,0), @sancion)
...
as
	...
	WHILE @cnt1 <= @NumLesiones
		BEGIN
			...
			SET @descripcion = ceatorresar.p_generar_descripcion_lesion(@num);
...
as
	...
	set @EquipoPuesto1 = (select top 1 equipo from v_equipo_partido where partido = 64 order by goles desc);

	declare @EquipoPuesto2 int;
	set @EquipoPuesto2 = (select top 1 equipo from v_equipo_partido where partido = 64 order by goles asc);

	declare @EquipoPuesto3 int;
	set @EquipoPuesto3 = (select top 1 equipo from v_equipo_partido where partido = 63 order by goles desc);

	declare @EquipoPuesto4 int;
	set @EquipoPuesto4 = (select top 1 equipo from v_equipo_partido where partido = 63 order by goles asc);
	...
	BEGIN
		...
		set @jugador = (
			select jugador from (
				select ROW_NUMBER() OVER(ORDER BY jugador DESC) AS fila, jugador from v_jugador_equipo where equipo = @EquipoPuesto1
			) t1 where fila = @cnt1
		)
		execute ingresarPremioJugador @jugador, 'Medalla de oro'

		set @jugador = (
			select jugador from (
				select ROW_NUMBER() OVER(ORDER BY jugador DESC) AS fila, jugador from v_jugador_equipo where equipo = @EquipoPuesto2
			) t1 where fila = @cnt1
		)
		execute ingresarPremioJugador @jugador, 'Medalla de plata'

		set @jugador = (
			select jugador from (
				select ROW_NUMBER() OVER(ORDER BY jugador DESC) AS fila, jugador from v_jugador_equipo where equipo = @EquipoPuesto3
			) t1 where fila = @cnt1
		)
		execute ingresarPremioJugador @jugador, 'Medalla de bronce'
	...
	SET @cnt1 = 1;
	declare @end int;
	set @end = (select count(*) from v_arbitro_partido where partido = 63 or partido = 64)
	WHILE @cnt1 <= @end
	BEGIN
		
		set @arbitro = (
			select distinct arbitro from (
				select distinct ROW_NUMBER() OVER(ORDER BY arbitro DESC) AS fila, arbitro from (
					select distinct arbitro from v_arbitro_partido where partido = 63 or partido = 64
				)t2
			) t1 where fila = @cnt1
		)
		execute ingresarPremioArbitro @arbitro, 'Medalla oficial'
		SET @cnt1 = @cnt1 + 1;
	END;

	declare @equipoFairPlay int;
	set @equipoFairPlay = ROUND(((32 - 1) * RAND()) + 1, 0);
	...
	BEGIN
		set @jugador = (
			select jugador from (
				select ROW_NUMBER() OVER(ORDER BY jugador DESC) AS fila, jugador from v_jugador_equipo where equipo = @equipoFairPlay
			) t1 where fila = @cnt1
		)
		execute ingresarPremioJugador @jugador, 'Medalla Fair Play'
	...
	SET @cnt1 = 1;
	set @end = (select count(*) from v_arbitros where pais = (select nombre from v_equipos where id_equipo = @equipoFairPlay))
	WHILE @cnt1 <= @end
	BEGIN		
		set @arbitro = (
			select arbitro from (
				select ROW_NUMBER() OVER(ORDER BY cedula DESC) AS fila, cedula as arbitro from v_arbitros where pais = (select nombre from v_equipos where id_equipo = @equipoFairPlay)
			) t1 where fila = @cnt1
		)
		execute ingresarPremioArbitro @arbitro, 'Medalla Fair Play'
		SET @cnt1 = @cnt1 + 1;
	END;

	set @jugador = ROUND((((select count(*) from v_jugador_equipo) - 1) * RAND()) + 1, 0);
...
as
	...
	BEGIN
		...
		declare @terreno int;
		set @terreno = round((20 - 1)*rand() + 1, 0);

		declare @fecha DATETIME;
		set @fecha = DATEADD(day,-1, (select fecha_hora from V_partidos where id_partido = @cnt1));
...
as
	...
	BEGIN
		...
		declare @estadio int;
		set @estadio = (select top 1 estadio from v_equipo_partido join V_partidos on partido = id_partido where partido = @cnt1 and posicion = 1);

		declare @fecha DATETIME;
		set @fecha = DATEADD(hour,-2, (select fecha_hora from V_partidos where id_partido = @cnt1));

		execute ingresarSesionCalentamiento      @equipo, @estadio, @fecha     

		set @equipo = (select top 1 equipo from v_equipo_partido where partido = @cnt1 and posicion = 2);
		set @estadio = (select top 1 estadio from v_equipo_partido join V_partidos on partido = id_partido where partido = @cnt1 and posicion = 2);
		set @fecha = DATEADD(hour,-1, (select fecha_hora from V_partidos where id_partido = @cnt1));
...
as
	...
	BEGIN
		...
		declare @equipo int;
		set @equipo = (select top 1 id_equipo from V_equipos order by newid());

		declare @valor int;
		set @valor = round(180000 * rand() + 20000, 0);

		declare @descripción varchar(255);
		set @descripción = ceatorresar.p_generar_descripcion_multa(round(4 * rand(), 0));
...
as
	...
	BEGIN
		...
		declare @jugador int;
		set @jugador = (
			select top 1 jugador from v_jugador_equipo 
...
as
	...
	BEGIN
		...
		set @fecha_hora = DATEADD(MINUTE, ROUND((90) * RAND(), 0), (select top 1 fecha_hora from V_partidos where id_partido = @partido));

		declare @resultado varchar(255);
		set @resultado = ceatorresar.p_generar_resultado_controlAntidopaje(round(rand(), 0));
...
as
	...
	BEGIN
		...
		declare @arbitro int;
		set @arbitro = (select top 1 cedula from v_arbitros left join V_arbitrossancionados on cedula = arbitro where arbitro is null order by newid());
